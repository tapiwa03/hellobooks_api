...
def register():
    ...
    if len(sent_data) is not 3:
        return jsonify({"message": "Please input your email, name and password"}), 400
    raw_data = {
        'name': sent_data['name'],
    ...
    if HelloBooks().user_data_validation(data) is True:
        username = data['name']
        email = data['email']
...
@jwt_required
def authorize():
    '''Function for authorizing a user to borrow books'''
    try:
...
def get_all_users():
    ...
    return User().get_all_users(page=parameters[0], per_page=parameters[1])
